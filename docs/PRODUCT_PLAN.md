...
| Activity library/manual logging | 61 activities, dated session-RPE log and strength sets, saved locally | Editing existing sessions, richer activity metadata, opt-in HealthKit writeback for manual logs |
...
| Sleep planner: peak/perform/get by | Editable wake time and three estimated sleep budgets | Persist schedule, infer circadian regularity, account for naps and shift work; currently not an alarm |
...
| Weekly Plan / Performance Assessment | Seven-day manual workload chart; coach can suggest a week in text | Persistent calendar, planned-vs-completed review, historical load/ramp metrics, confidence-aware weekly report |
...
| Health Monitor | Latest HRV/RHR/respiration/wrist temperature/oxygen/VO₂ max with sample time/source | Source-specific ranges, outlier handling, all-vital charts, normalized 30/180-day comparisons. Readings are not all live |
