...
GitHub Actions now runs the XCTest suite and a full unsigned iPhone/embedded Watch simulator build on `macos-26`. See the repository Native build workflow for the latest outcome. This does not test hardware sensors, live model availability, signing or installation.
